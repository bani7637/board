...
<script type="text/javascript">
	...
	$(function() {
		...
		if (getCookieValues("REMEMBERME") == "Y") {
			...
			// 3. usernm확인후 값설정 
			getCookieValues("mem_id")
			console.log(getCookieValues("mem_id"));
			document.getElementById('inputEmail').value = getCookieValues("mem_id")
		}
		...
		$('#Signin').on('click', function() {
			if ($("input:checkbox[id='remember']").prop("checked") == true) {
				...
				setCookie('mem_id', $('#inputEmail').val());
			} else {
...
<body>
	...
	<div class="container">
		...
		<form class="form-signin" action="${pageContext.request.contextPath}/login" method="post">
			...
			<input type="email" id="inputEmail" name="mem_id" value="bani" class="form-control" placeholder="Email address" required autofocus>
			<label for="inputPassword" class="sr-only" >Password</label>
			<input type="password" id="inputPassword" required value="123" class="form-control" placeholder="Password" required name="mem_pass">
			<div class="checkbox">
				<label> <input type="checkbox" value="remember-me" id="remember" checked>Remember me</label>
			</div>
